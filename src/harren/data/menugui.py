...
class QuickStats(pg.sprite.Sprite):
    def __init__(self, game_data):
        ...
        self.font = pg.font.Font(
            data.setup.FONTS[data.constants.MAIN_FONT],
            22
        )
    ...
    def make_image(self):
        ...
        image = data.setup.GFX['goldbox']
        rect = image.get_rect(left=10, top=244)

        surface = pg.Surface(rect.size)
        surface.set_colorkey(data.constants.BLACK)
        ...
        for i, stat in enumerate(stat_list):
            ...
            if stat in magic_health_list:
                text = "{}{}: {}/{}".format(
                    first_letter,
                    rest_of_letters,
                    self.stats[stat]['current'],
                    self.stats[stat]['maximum']
                )
            elif stat == 'GOLD':
...
class InfoBox(pg.sprite.Sprite):
    def __init__(self, inventory, player_stats):
        ...
        self.get_tile = data.tools.get_tile
        self.sword = self.get_tile(
            48,
            0,
            data.setup.GFX['shopsigns'],
            16,
            16,
            2
        )
        self.shield = self.get_tile(
            32,
            0,
            data.setup.GFX['shopsigns'],
            16,
        ...
        for i, stat in enumerate(stat_list):
            if stat == 'health' or stat == 'magic':
                text = "{}{}: {} / {}".format(
                    stat[0].upper(),
                    stat[1:],
                    self.player_stats[stat]['current'],
                    self.player_stats[stat]['maximum']
                )
...
class MenuGui(object):
    def __init__(self, level, inventory, stats):
        ...
        self.sfx_observer = data.observer.SoundEffects()
        ...
        if self.allow_input:
            if keys[pg.K_DOWN]:
                if self.arrow_index < len(self.arrow.pos_list) - 1:
                    self.notify(data.constants.CLICK)
                    self.arrow_index += 1
                    self.allow_input = False
            elif keys[pg.K_UP]:
                ...
                self.notify(data.constants.CLICK2)
    ...
    def use_cure_spell(self):
        ...
        if health['current'] != health['maximum']:
            if magic['current'] >= inventory['Cure']['magic points']:
                self.notify(data.constants.POWERUP)
