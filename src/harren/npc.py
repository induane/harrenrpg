...
class NPC(pg.sprite.Sprite):
    ...
    __slots__ = ("data", "game_loop", "image", "rect", "npc_data", "name", "base_dialog", "alt_dialog")
    ...
    def interact(self):
        ...
        if intersect:
            ...
            for key, value in active_data.items():
                ...
                if key in ("start_quest", "inventory", "shadow_items", "quest_complete", "dialog", "alt_dialog"):
                    continue
                ...
                if key in quest_inventory or key in inventory:
                    ...
                    if value.get("quest_complete", False) is True:
                        ...
                        self.game_loop.quests.pop(self.game_loop.quests.index(current_quest))
                        self.game_loop.completed_quests.append(current_quest)

                        dialog = value.get("dialog") or []
                        if not dialog:
                            LOG.warning("NPC %s missing some quest dialog", self.name)
            ...
            if shadow_items:
                ...
                if not dialog:
                    LOG.warning("Quest %s missing dialog for npc %s", current_quest, self.name)
                if isinstance(dialog, str):
                    dialog = [
                        dialog,
                    ]
        ...
        for quest_name, quest in npc_data.items():
            ...
            if quest.get("start_quest", False) is True:
                ...
                self.game_loop.notification = "New quest! {}".format(quest_info.get("name", quest_name) or quest_name)
                return dialog[:]
