...
class BaseLevel:
    def __init__(self, filename, game_loop, **kwargs):
        ...
        self.scroll_group = PyscrollGroup(map_layer=self.map_layer, default_layer=10)
        if self.player1:
    ...
    def draw_notifications(self, surface, viewport):
        ...
        notification_text = self.font_25.render(notification, True, (255, 255, 255))
            ...
            if is_teleporting:
                ...
                if teleport_target:
                    for p_target in self.custom_objects["portal_targets"]:
                        ...
                        LOG.warning("Could not find target portal %s", teleport_target)
        return player1
    ...
    def custom_objects(self):
        ...
        for obj in self.tmx_data.objects:
            ...
            if asset_type == "blocker" or name == "blocker":
                ...
                portal_targets.append({"name": name, "rect": pg_rect(obj.x, obj.y, 16, 16)})
                ...
                if not destination:
                    LOG.warning("Portal at %s, %s missing destination.", obj.x, obj.y)
                else:
                    portals.append(
                        {
                            "name": name,
                ...
                sprite = custom_properties.get("sprite")
                npcs.append(NPC(self.game_loop, sprite, pg_rect(obj.x, obj.y, 16, 16), data=custom_properties))
