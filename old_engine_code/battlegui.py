...
class InfoBox(object):
    ...
    def make_magic_text(self):
        ...
        spell_list = [x for x in inventory if x in allowed_item_list]
...
class SelectArrow(object):
    ...
    def select_action(self, keys):
        ...
        if self.index > (len(self.pos_list) - 1):
            LOG.debug('Select action index %s, %s', self.pos_list, self.index)
        self.rect.topleft = self.pos_list[self.index]
    ...
    def check_input(self, keys):
        ...
        if keys[pg.K_DOWN] == 0 and keys[pg.K_UP] == 0 and keys[pg.K_RIGHT] == 0 and keys[pg.K_LEFT] == 0:
